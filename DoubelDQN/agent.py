...
class DQN:
    ...
    def update(self):
        ...
        #1、计算Q_target 区别于 一般DQN的地方
        with torch.no_grad():
            next_q_policy = self.policy_net(next_state_batch)
            selected_nxt_actions = torch.argmax(next_q_policy, dim=1).view(-1, 1)
            Q_t_plus_one = self.target_net(next_state_batch).gather(dim=1, index=selected_nxt_actions).squeeze()
