...
from A2C.agent0 import A2CTrainer_v0
from A2C.agent1 import A2CTrainer_v1
...
class A2CConfig:
    def __init__(self):
        ...
        self.train_eps = 500
        #self.train_eps = 5000 agentv1的训练比较不稳定
        self.eval_eps = 50
        self.gamma = 0.99  # gamma: Gamma discount factor.
        self.lr = 0.001  # learning rate: step size parameter
        self.clip_gradient = None
        self.update_freq = 10
        self.num_critic_updates = 10
        self.num_critic_update_steps = 10
...
def env_agent_config(config, seed=1):
    ...
    #agent = A2CTrainer_v1(state_dim, act_dim, config)
    agent = A2CTrainer_v0(state_dim, act_dim, config)
    return env, agent
